  ...
  const [brands, setBrands] = useState([]);
    ...
    useEffect(() => {
        const fetchCollections = async () => {
            try {
                const res = await fetch("/umbraco/delivery/api/v2/content?filter=contentType%3AcollectionPage");
    ...
    useEffect(() => {
        const fetchBrands = async () => {
            try {
                const res = await fetch("/umbraco/delivery/api/v2/content?filter=contentType%3AproductPage&skip=0&take=10000&fields=properties%5Bbrand%5D");
                const data = await res.json();
    ...
    const fetchFemeiLinks = async () => {
      try {
        const res = await fetch("/umbraco/delivery/api/v2/content?filter=contentType%3AcategoryPage&skip=0&take=400");
        const data = await res.json();
