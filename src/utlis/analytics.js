...
export const handleGlobalProductClick = async (productId, title) => {
    try {
        const res = await fetch('https://api.indulap.ro/umbraco/delivery/api/productclicks/increment', {
            method: 'POST',
            headers: {
...
export const getTodaysTopClickedProducts = async () => {
    const res = await fetch("https://api.indulap.ro/umbraco/delivery/api/productclicks/top?top=4&period=today");
    return res.json();
};

export const getAllTimeTopClickedProducts = async () => {
    const res = await fetch("https://api.indulap.ro/umbraco/delivery/api/productclicks/top?top=4&period=alltime");
    return res.json();
};
